...
class modelObj:
    ...
    def deform_net(self, batch_size):
        ...
        return {'x_tmp': x_tmp, 'flow_v': v_tmp, 'deform_x': w_tmp, 'y_tmp': y_tmp, 'y_tmp_1hot': y_tmp_1hot,
                'deform_y_1hot': w_tmp_1hot}
    ...
    def encoder_network(self, x, train_phase, no_filters, encoder_list_return=0):
        ...
        # Level 1
        enc_c1_a = layers.conv2d_layer(ip_layer=x, name='enc_c1_a', num_filters=no_filters[1], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c1_b = layers.conv2d_layer(ip_layer=enc_c1_a, name='enc_c1_b', num_filters=no_filters[1], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c1_pool = layers.max_pool_layer2d(enc_c1_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",
                                              name='enc_c1_pool')
        enc_layers_list.append(enc_c1_b)

        # Level 2
        enc_c2_a = layers.conv2d_layer(ip_layer=enc_c1_pool, name='enc_c2_a', num_filters=no_filters[2], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c2_b = layers.conv2d_layer(ip_layer=enc_c2_a, name='enc_c2_b', num_filters=no_filters[2], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c2_pool = layers.max_pool_layer2d(enc_c2_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",
                                              name='enc_c2_pool')
        enc_layers_list.append(enc_c2_b)

        # Level 3
        enc_c3_a = layers.conv2d_layer(ip_layer=enc_c2_pool, name='enc_c3_a', num_filters=no_filters[3], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c3_b = layers.conv2d_layer(ip_layer=enc_c3_a, name='enc_c3_b', num_filters=no_filters[3], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c3_pool = layers.max_pool_layer2d(enc_c3_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",
                                              name='enc_c3_pool')
        enc_layers_list.append(enc_c3_b)

        # Level 4
        enc_c4_a = layers.conv2d_layer(ip_layer=enc_c3_pool, name='enc_c4_a', num_filters=no_filters[4], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c4_b = layers.conv2d_layer(ip_layer=enc_c4_a, name='enc_c4_b', num_filters=no_filters[4], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c4_pool = layers.max_pool_layer2d(enc_c4_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",
                                              name='enc_c4_pool')
        enc_layers_list.append(enc_c4_b)

        # Level 5 - 2x Conv
        enc_c5_a = layers.conv2d_layer(ip_layer=enc_c4_pool, name='enc_c5_a', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c5_b = layers.conv2d_layer(ip_layer=enc_c5_a, name='enc_c5_b', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c5_pool = layers.max_pool_layer2d(enc_c5_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",
                                              name='enc_c5_pool')
        enc_layers_list.append(enc_c5_b)

        # Level 6 - 2x Conv
        enc_c6_a = layers.conv2d_layer(ip_layer=enc_c5_pool, name='enc_c6_a', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        enc_c6_b = layers.conv2d_layer(ip_layer=enc_c6_a, name='enc_c6_b', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
    ...
    def encoder_pretrain_net(self, learn_rate_seg=0.001, temp_fac=0.1, global_loss_exp_no=1, n_parts=4):
        ...
        if (global_loss_exp_no == 1):
            ...
            for pos_index in range(0, self.batch_size, 1):
                ...
                num_i1_i2_loss = -tf.log(
                    tf.exp(num_i1_i2_ss) / (tf.exp(num_i1_i2_ss) + tf.math.reduce_sum(tf.exp(den_i1_i2_ss))))
                ...
                num_i2_i1_loss = -tf.log(
                    tf.exp(num_i1_i2_ss) / (tf.exp(num_i1_i2_ss) + tf.math.reduce_sum(tf.exp(den_i2_i1_ss))))
                ...
                num_i1_i3_loss = -tf.log(
                    tf.exp(num_i1_i3_ss) / (tf.exp(num_i1_i3_ss) + tf.math.reduce_sum(tf.exp(den_i1_i3_ss))))
                ...
                num_i3_i1_loss = -tf.log(
                    tf.exp(num_i1_i3_ss) / (tf.exp(num_i1_i3_ss) + tf.math.reduce_sum(tf.exp(den_i3_i1_ss))))
                ...
                num_i2_i3_loss = -tf.log(
                    tf.exp(num_i2_i3_ss) / (tf.exp(num_i2_i3_ss) + tf.math.reduce_sum(tf.exp(den_i2_i3_ss))))
                ...
                num_i3_i2_loss = -tf.log(
                    tf.exp(num_i2_i3_ss) / (tf.exp(num_i2_i3_ss) + tf.math.reduce_sum(tf.exp(den_i3_i2_ss))))
            ...
            for pos_index in range(0, bs, 1):
                ...
                if (pos_index % (2 * n_parts) < n_parts):
                    ...
                    num_i3_i4_loss = -tf.log(
                        tf.exp(num_i3_i4_ss) / (tf.exp(num_i3_i4_ss) + tf.math.reduce_sum(tf.exp(den_i3_i4_ss))))
                    ...
                    num_i4_i3_loss = -tf.log(
                        tf.exp(num_i3_i4_ss) / (tf.exp(num_i3_i4_ss) + tf.math.reduce_sum(tf.exp(den_i4_i3_ss))))
                    ...
                    num_i2_i4_loss = -tf.log(
                        tf.exp(num_i2_i4_ss) / (tf.exp(num_i2_i4_ss) + tf.math.reduce_sum(tf.exp(den_i2_i4_ss))))
                    ...
                    num_i4_i2_loss = -tf.log(
                        tf.exp(num_i2_i4_ss) / (tf.exp(num_i2_i4_ss) + tf.math.reduce_sum(tf.exp(den_i4_i2_ss))))
            ...
            for pos_index in range(0, bs, 1):
                ...
                if (pos_index % (3 * n_parts) < n_parts):
                    ...
                    num_i1_i4_loss = -tf.log(
                        tf.exp(num_i1_i4_ss) / (tf.exp(num_i1_i4_ss) + tf.math.reduce_sum(tf.exp(den_i1_i4_ss))))
                    net_global_loss = net_global_loss + num_i1_i4_loss
                    # for positive pair (x_j1, x_i1)
                    # numerator same & denominator of loss term (den_i3_i2_ss) & loss (num_i3_i2_loss)
                    den_i4_i1_ss = self.cos_sim(x_num_i4, x_den, temp_fac)
                    num_i4_i1_loss = -tf.log(
                        tf.exp(num_i1_i4_ss) / (tf.exp(num_i1_i4_ss) + tf.math.reduce_sum(tf.exp(den_i4_i1_ss))))
                    ...
                    num_i4_i5_loss = -tf.log(
                        tf.exp(num_i4_i5_ss) / (tf.exp(num_i4_i5_ss) + tf.math.reduce_sum(tf.exp(den_i4_i5_ss))))
                    net_global_loss = net_global_loss + num_i4_i5_loss
                    # for positive pair (x_a_j1, x_j1)
                    # numerator same & denominator of loss term (den_i4_i2_ss) & loss (num_i4_i2_loss)
                    den_i5_i4_ss = self.cos_sim(x_num_i5, x_den, temp_fac)
                    num_i5_i4_loss = -tf.log(
                        tf.exp(num_i4_i5_ss) / (tf.exp(num_i4_i5_ss) + tf.math.reduce_sum(tf.exp(den_i5_i4_ss))))
                    ...
                    num_i5_i6_loss = -tf.log(
                        tf.exp(num_i5_i6_ss) / (tf.exp(num_i5_i6_ss) + tf.math.reduce_sum(tf.exp(den_i5_i6_ss))))
                    ...
                    num_i6_i5_loss = -tf.log(
                        tf.exp(num_i5_i6_ss) / (tf.exp(num_i5_i6_ss) + tf.math.reduce_sum(tf.exp(den_i6_i5_ss))))
                    ...
                    num_i2_i5_loss = -tf.log(
                        tf.exp(num_i2_i5_ss) / (tf.exp(num_i2_i5_ss) + tf.math.reduce_sum(tf.exp(den_i2_i5_ss))))
                    net_global_loss = net_global_loss + num_i2_i5_loss
                    # for positive pair (x_a_j1, x_a_i1)
                    # numerator same & denominator of loss term (den_i4_i2_ss) & loss (num_i4_i2_loss)
                    den_i5_i2_ss = self.cos_sim(x_num_i5, x_den, temp_fac)
                    num_i5_i2_loss = -tf.log(
                        tf.exp(num_i2_i5_ss) / (tf.exp(num_i2_i5_ss) + tf.math.reduce_sum(tf.exp(den_i5_i2_ss))))
                    ...
                    num_i3_i6_loss = -tf.log(
                        tf.exp(num_i3_i6_ss) / (tf.exp(num_i3_i6_ss) + tf.math.reduce_sum(tf.exp(den_i3_i6_ss))))
                    ...
                    num_i6_i3_loss = -tf.log(
                        tf.exp(num_i3_i6_ss) / (tf.exp(num_i3_i6_ss) + tf.math.reduce_sum(tf.exp(den_i6_i3_ss))))
        ...
        for v in tf.trainable_variables():
            var_name = v.name
            if 'enc_' in var_name:
                reg_net_vars.append(v)
            elif 'dec_' in var_name:
                reg_net_vars.append(v)
            elif 'reg_' in var_name:
                reg_net_vars.append(v)
    ...
    def seg_unet(self, learn_rate_seg=0.001, dsc_loss=2, en_1hot=0, mtask_en=1, fs_de=2):
        ...
        x = tf.placeholder(tf.float32, shape=[None, self.img_size_x, self.img_size_y, num_channels], name='x')
        if (en_1hot == 1):
            y_l = tf.placeholder(tf.float32, shape=[None, self.img_size_x, self.img_size_y, self.num_classes],
                                 name='y_l')
        else:
        ...
        dec_dc6 = layers.conv2d_layer(ip_layer=dec_c6_up, name='dec_dc6', kernel_size=(fs_de, fs_de),
                                      num_filters=no_filters[5], use_relu=True, use_batch_norm=True,
                                      training_phase=train_phase)
        dec_cat_c6 = tf.concat((dec_dc6, enc_c5_b), axis=3, name='dec_cat_c6')
        dec_c5_a = layers.conv2d_layer(ip_layer=dec_cat_c6, name='dec_c5_a', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        dec_c5_b = layers.conv2d_layer(ip_layer=dec_c5_a, name='dec_c5_b', num_filters=no_filters[5], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        ...
        dec_dc5 = layers.conv2d_layer(ip_layer=dec_c5_up, name='dec_dc5', kernel_size=(fs_de, fs_de),
                                      num_filters=no_filters[4], use_relu=True, use_batch_norm=True,
                                      training_phase=train_phase)
        dec_cat_c5 = tf.concat((dec_dc5, enc_c4_b), axis=3, name='dec_cat_c5')
        dec_c4_a = layers.conv2d_layer(ip_layer=dec_cat_c5, name='dec_c4_a', num_filters=no_filters[4], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        dec_c4_b = layers.conv2d_layer(ip_layer=dec_c4_a, name='dec_c4_b', num_filters=no_filters[4], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)

        # Level 4
        dec_up4 = layers.upsample_layer(ip_layer=dec_c4_b, method=self.interp_val, scale_factor=scale_fac)
        dec_dc4 = layers.conv2d_layer(ip_layer=dec_up4, name='dec_dc4', kernel_size=(fs_de, fs_de),
                                      num_filters=no_filters[3], use_relu=True, use_batch_norm=True,
                                      training_phase=train_phase)
        dec_cat_c4 = tf.concat((dec_dc4, enc_c3_b), axis=3, name='dec_cat_c4')
        dec_c3_a = layers.conv2d_layer(ip_layer=dec_cat_c4, name='dec_c3_a', num_filters=no_filters[3], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        dec_c3_b = layers.conv2d_layer(ip_layer=dec_c3_a, name='dec_c3_b', num_filters=no_filters[3], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)

        # Level 3
        dec_up3 = layers.upsample_layer(ip_layer=dec_c3_b, method=self.interp_val, scale_factor=scale_fac)
        dec_dc3 = layers.conv2d_layer(ip_layer=dec_up3, name='dec_dc3', kernel_size=(fs_de, fs_de),
                                      num_filters=no_filters[2], use_relu=True, use_batch_norm=True,
                                      training_phase=train_phase)
        dec_cat_c3 = tf.concat((dec_dc3, enc_c2_b), axis=3, name='dec_cat_c3')
        dec_c2_a = layers.conv2d_layer(ip_layer=dec_cat_c3, name='dec_c2_a', num_filters=no_filters[2], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        dec_c2_b = layers.conv2d_layer(ip_layer=dec_c2_a, name='dec_c2_b', num_filters=no_filters[2], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)

        # Level 2
        dec_up2 = layers.upsample_layer(ip_layer=dec_c2_b, method=self.interp_val, scale_factor=scale_fac)
        dec_dc2 = layers.conv2d_layer(ip_layer=dec_up2, name='dec_dc2', kernel_size=(fs_de, fs_de),
                                      num_filters=no_filters[1], use_relu=True, use_batch_norm=True,
                                      training_phase=train_phase)
        dec_cat_c2 = tf.concat((dec_dc2, enc_c1_b), axis=3, name='dec_cat_c2')
        dec_c1_a = layers.conv2d_layer(ip_layer=dec_cat_c2, name='dec_c1_a', num_filters=no_filters[1], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)

        # Level 1
        seg_c1_a = layers.conv2d_layer(ip_layer=dec_c1_a, name='seg_c1_a', num_filters=no_filters[1], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)
        seg_c1_b = layers.conv2d_layer(ip_layer=seg_c1_a, name='seg_c1_b', num_filters=no_filters[1], use_relu=True,
                                       use_batch_norm=True, training_phase=train_phase)

        # Final output layer - Logits before softmax
        seg_fin_layer = layers.conv2d_layer(ip_layer=seg_c1_b, name='seg_fin_layer', num_filters=self.num_classes,
                                            use_bias=False, use_relu=False, use_batch_norm=False,
                                            training_phase=train_phase)
        ...
        if (dsc_loss == 1):
            ...
            seg_cost = loss.pixel_wise_cross_entropy_loss_weighted(logits=seg_fin_layer, labels=y_l_onehot,
        ...
        for v in tf.trainable_variables():
            var_name = v.name
            if 'enc_' in var_name:
                all_net_vars.append(v)
            elif 'dec_' in var_name:
                all_net_vars.append(v)
            elif 'seg_' in var_name:
                all_net_vars.append(v)
        ...
        with tf.control_dependencies(update_ops):
            ...
            optimizer_unet_seg = tf.train.AdamOptimizer(learning_rate=learn_rate_seg).minimize(cost_seg,
                                                                                               var_list=seg_net_vars)
            optimizer_unet_dec = tf.train.AdamOptimizer(learning_rate=learn_rate_seg).minimize(cost_seg,
                                                                                               var_list=dec_net_vars)
            optimizer_unet_all = tf.train.AdamOptimizer(learning_rate=learn_rate_seg).minimize(cost_seg,
                                                                                               var_list=all_net_vars)
        ...
        if (mtask_en == 1):
            return {'x': x, 'y_l': y_l, 'train_phase': train_phase, 'seg_cost': cost_seg,
            ...
            return {'x': x, 'y_l': y_l, 'train_phase': train_phase, 'seg_cost': cost_seg,
                    ...
                    'train_summary': train_summary, 'seg_fin_layer': seg_fin_layer,
                    'optimizer_unet_all': optimizer_unet_all, \
                    'actual_cost': actual_cost, 'mean_dice': mean_dice, 'val_totalc': val_totalc,
                    'val_summary': val_summary}

    def decoder_pretrain_net(self, learn_rate_seg=0.001, temp_fac=1, no_of_local_regions=5, fs_de=2,
                             no_of_decoder_blocks=1, local_reg_size=0, \
                             wgt_en=0, no_of_neg_local_regions=5, local_loss_exp_no=0, no_of_neg_regs_override=5,
                             inf=0):
        ...
        if (no_of_decoder_blocks >= 1):
            ...
            if (no_of_decoder_blocks >= 2):
                ...
                dec_c5_up = layers.upsample_layer(ip_layer=dec_c5_b, method=self.interp_val,
                                                  scale_factor=int(scale_fac))
                dec_dc5 = layers.conv2d_layer(ip_layer=dec_c5_up, name='dec_dc5', kernel_size=(fs_de, fs_de),
                                              num_filters=no_filters[4], use_relu=True, use_batch_norm=True,
                                              training_phase=train_phase)
                dec_cat_c5 = tf.concat((dec_dc5, enc_c4_b), axis=3, name='dec_cat_c5')
                dec_c4_a = layers.conv2d_layer(ip_layer=dec_cat_c5, name='dec_c4_a', num_filters=no_filters[4],
                                               use_relu=True, use_batch_norm=True, training_phase=train_phase)
                dec_c4_b = layers.conv2d_layer(ip_layer=dec_c4_a, name='dec_c4_b', num_filters=no_filters[4],
                                               use_relu=True, use_batch_norm=True, training_phase=train_phase)
                ...
                if (no_of_decoder_blocks >= 3):
                    ...
                    dec_dc4 = layers.conv2d_layer(ip_layer=dec_up4, name='dec_dc4', kernel_size=(fs_de, fs_de),
                                                  ...
                                                  training_phase=train_phase)
                    dec_cat_c4 = tf.concat((dec_dc4, enc_c3_b), axis=3, name='dec_cat_c4')
                    dec_c3_a = layers.conv2d_layer(ip_layer=dec_cat_c4, name='dec_c3_a', num_filters=no_filters[3],
                                                   use_relu=True, use_batch_norm=True, training_phase=train_phase)
                    dec_c3_b = layers.conv2d_layer(ip_layer=dec_c3_a, name='dec_c3_b', num_filters=no_filters[3],
                                                   use_relu=True, use_batch_norm=True, training_phase=train_phase)
                    ...
                    if (no_of_decoder_blocks >= 4):
                        ...
                        dec_up3 = layers.upsample_layer(ip_layer=dec_c3_b, method=self.interp_val,
                                                        scale_factor=scale_fac)
                        dec_dc3 = layers.conv2d_layer(ip_layer=dec_up3, name='dec_dc3', kernel_size=(fs_de, fs_de),
                                                      num_filters=no_filters[2], use_relu=True, use_batch_norm=True,
                                                      training_phase=train_phase)
                        dec_cat_c3 = tf.concat((dec_dc3, enc_c2_b), axis=3, name='dec_cat_c3')
                        dec_c2_a = layers.conv2d_layer(ip_layer=dec_cat_c3, name='dec_c2_a', num_filters=no_filters[2],
                                                       use_relu=True, use_batch_norm=True, training_phase=train_phase)
                        dec_c2_b = layers.conv2d_layer(ip_layer=dec_c2_a, name='dec_c2_b', num_filters=no_filters[2],
                                                       use_relu=True, use_batch_norm=True, training_phase=train_phase)
                        ...
                        if (no_of_decoder_blocks >= 5):
                            ...
                            dec_up2 = layers.upsample_layer(ip_layer=dec_c2_b, method=self.interp_val,
                                                            scale_factor=scale_fac)
                            dec_dc2 = layers.conv2d_layer(ip_layer=dec_up2, name='dec_dc2', kernel_size=(fs_de, fs_de),
                                                          num_filters=no_filters[1], use_relu=True, use_batch_norm=True,
                                                          training_phase=train_phase)
                            dec_cat_c2 = tf.concat((dec_dc2, enc_c1_b), axis=3, name='dec_cat_c2')
                            dec_c1_a = layers.conv2d_layer(ip_layer=dec_cat_c2, name='dec_c1_a',
                                                           num_filters=no_filters[1], use_relu=True,
                                                           use_batch_norm=True, training_phase=train_phase)
        ...
        seg_c1_a = layers.conv2d_layer(ip_layer=tmp_dec_layer, name='seg_c1_a', kernel_size=(1, 1),
                                       num_filters=tmp_no_filters, use_bias=False, use_relu=True, use_batch_norm=True,
                                       training_phase=train_phase)
        seg_c1_b = layers.conv2d_layer(ip_layer=seg_c1_a, name='seg_c1_b', kernel_size=(1, 1),
                                       num_filters=tmp_no_filters, use_bias=False, use_relu=False, use_batch_norm=False,
                                       training_phase=train_phase)
        ...
        if (no_of_local_regions == 9):
            ...
            if (local_reg_size == 1):
                ...
                neg_sample_indexes[4, :, :] = [[int(im_x / 4), int(im_y / 4)], [int(im_x / 4), int(3 * im_y / 4)],
                                               ...
                                               [int(3 * im_x / 4), int(3 * im_y / 4)], [int(im_x / 2), 0]]
                ...
                neg_sample_indexes[6, :, :] = [[int(im_x / 2), 0], [int(im_x / 2), int(im_y / 4)],
                                               [im_x, int(im_y / 2)], [int(3 * im_x / 4), int(im_y / 4)],
                ...
                neg_sample_indexes[2, :, :] = [[0, int(im_y / 2)], [int(im_x / 2), int(im_y / 2)],
                                               [int(im_x / 2), im_y], [int(3 * im_x / 4), int(im_y / 4)],
                                               [int(im_x / 4), int(3 * im_y / 4)]]
            ...
            for pos_index in range(0, bs, 2):
                ...
                if (local_reg_size == 1):
                    ...
                    for local_pos_index in range(0, no_of_local_regions, 1):
                        ...
                        x_num_tmp_i1 = tf.gather(x_num_i1, [pos_sample_indexes[local_pos_index, 0],
                                                            pos_sample_indexes[local_pos_index, 0] + 1,
                                                            pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_num_tmp_i1 = tf.gather(x_num_tmp_i1, [pos_sample_indexes[local_pos_index, 1],
                                                                pos_sample_indexes[local_pos_index, 1] + 1,
                                                                pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i1_flat = tf.layers.flatten(inputs=x_num_tmp_i1)
                        if (wgt_en == 1):
                            x_w3_n_i1 = tf.layers.dense(inputs=x_n_i1_flat, units=128, name='seg_pred', activation=None,
                        ...
                        # fetch x and y coordinates
                        x_num_tmp_i2 = tf.gather(x_num_i2, [pos_sample_indexes[local_pos_index, 0],
                                                            pos_sample_indexes[local_pos_index, 0] + 1,
                                                            pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_num_tmp_i2 = tf.gather(x_num_tmp_i2, [pos_sample_indexes[local_pos_index, 1],
                                                                pos_sample_indexes[local_pos_index, 1] + 1,
                                                                pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i2_flat = tf.layers.flatten(inputs=x_num_tmp_i2)
                        if (wgt_en == 1):
                            x_w3_n_i2 = tf.layers.dense(inputs=x_n_i2_flat, units=128, name='seg_pred', activation=None,
                                                        use_bias=False, reuse=tf.AUTO_REUSE)
                        else:
                        ...
                        for local_neg_index in range(0, no_of_neg_pts, 1):
                            ...
                            x_den_tmp_i1 = tf.gather(x_num_i1, [neg_samples_index_list[local_neg_index, 0],
                                                                neg_samples_index_list[local_neg_index, 0] + 1,
                                                                neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_den_tmp_i1 = tf.gather(x_den_tmp_i1, [neg_samples_index_list[local_neg_index, 1],
                            ...
                            if (wgt_en == 1):
                                x_w3_d_i1 = tf.layers.dense(inputs=x_d_i1_flat, units=128, name='seg_pred',
                                                            activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                                x_w3_d_i1 = x_d_i1_flat

                            # negative local regions in feature map (f_a2_i) from image (x_a2_i)
                            x_den_tmp_i2 = tf.gather(x_num_i2, [neg_samples_index_list[local_neg_index, 0],
                                                                neg_samples_index_list[local_neg_index, 0] + 1,
                                                                neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_den_tmp_i2 = tf.gather(x_den_tmp_i2, [neg_samples_index_list[local_neg_index, 1],
                                                                    neg_samples_index_list[local_neg_index, 1] + 1,
                                                                    neg_samples_index_list[local_neg_index, 1] + 2],
                                                     axis=2)
                            x_d_i2_flat = tf.layers.flatten(inputs=x_den_tmp_i2)
                            if (wgt_en == 1):
                                x_w3_d_i2 = tf.layers.dense(inputs=x_d_i2_flat, units=128, name='seg_pred',
                                                            activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                        ...
                        num_i1_loss = -tf.log(tf.math.reduce_sum(tf.exp(num_i1_ss)) / (
                                    tf.math.reduce_sum(tf.exp(num_i1_ss)) + tf.math.reduce_sum(den_i1_ss)))
                        ...
                        num_i2_loss = -tf.log(tf.math.reduce_sum(tf.exp(num_i2_ss)) / (
                                    tf.math.reduce_sum(tf.exp(num_i2_ss)) + tf.math.reduce_sum(den_i2_ss)))
            ...
            for pos_index in range(0, tmp_batch_size, 1):
                ...
                for local_pos_index in range(0, no_of_local_regions, 1):
                    ...
                    if (local_reg_size == 1):
                        ...
                        x_n_tmp_i1 = tf.gather(x_num_i1, [pos_sample_indexes[local_pos_index, 0],
                                                          pos_sample_indexes[local_pos_index, 0] + 1,
                                                          pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_n_tmp_i1 = tf.gather(x_n_tmp_i1, [pos_sample_indexes[local_pos_index, 1],
                                                            pos_sample_indexes[local_pos_index, 1] + 1,
                                                            pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i1_flat = tf.layers.flatten(inputs=x_n_tmp_i1)
                        if (wgt_en == 1):
                            x_num_tmp_i1 = tf.layers.dense(inputs=x_n_i1_flat, units=128, name='seg_pred',
                                                           activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                        else:
                        ...
                        # fetch x and y coordinates of local region
                        x_n_tmp_i2 = tf.gather(x_num_i2, [pos_sample_indexes[local_pos_index, 0],
                                                          pos_sample_indexes[local_pos_index, 0] + 1,
                                                          pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_n_tmp_i2 = tf.gather(x_n_tmp_i2, [pos_sample_indexes[local_pos_index, 1],
                                                            pos_sample_indexes[local_pos_index, 1] + 1,
                                                            pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i2_flat = tf.layers.flatten(inputs=x_n_tmp_i2)
                        if (wgt_en == 1):
                            x_num_tmp_i2 = tf.layers.dense(inputs=x_n_i2_flat, units=128, name='seg_pred',
                                                           activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                        else:
                        ...
                        # fetch x and y coordinates of local region
                        x_n_tmp_i3 = tf.gather(x_num_i3, [pos_sample_indexes[local_pos_index, 0],
                                                          pos_sample_indexes[local_pos_index, 0] + 1,
                                                          pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_n_tmp_i3 = tf.gather(x_n_tmp_i3, [pos_sample_indexes[local_pos_index, 1],
                                                            pos_sample_indexes[local_pos_index, 1] + 1,
                                                            pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i3_flat = tf.layers.flatten(inputs=x_n_tmp_i3)
                        if (wgt_en == 1):
                            x_num_tmp_i3 = tf.layers.dense(inputs=x_n_i3_flat, units=128, name='seg_pred',
                                                           activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                        else:
                        ...
                        # fetch x and y coordinates of local region
                        x_n_tmp_i4 = tf.gather(x_num_i4, [pos_sample_indexes[local_pos_index, 0],
                                                          pos_sample_indexes[local_pos_index, 0] + 1,
                                                          pos_sample_indexes[local_pos_index, 0] + 2], axis=1)
                        x_n_tmp_i4 = tf.gather(x_n_tmp_i4, [pos_sample_indexes[local_pos_index, 1],
                                                            pos_sample_indexes[local_pos_index, 1] + 1,
                                                            pos_sample_indexes[local_pos_index, 1] + 2], axis=2)
                        x_n_i4_flat = tf.layers.flatten(inputs=x_n_tmp_i4)
                        if (wgt_en == 1):
                            x_num_tmp_i4 = tf.layers.dense(inputs=x_n_i4_flat, units=128, name='seg_pred',
                                                           activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                        else:
                    ...
                    for local_neg_index in range(0, no_of_neg_pts, 1):
                        if (local_reg_size == 1):
                            # negative local regions in feature map (f_a1_i) from image (x_a1_i)
                            x_d_tmp_i1 = tf.gather(x_num_i1, [neg_samples_index_list[local_neg_index, 0],
                                                              neg_samples_index_list[local_neg_index, 0] + 1,
                                                              neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_d_tmp_i1 = tf.gather(x_d_tmp_i1, [neg_samples_index_list[local_neg_index, 1],
                                                                neg_samples_index_list[local_neg_index, 1] + 1,
                                                                neg_samples_index_list[local_neg_index, 1] + 2], axis=2)
                            x_d_i1_flat = tf.layers.flatten(inputs=x_d_tmp_i1)
                            if (wgt_en == 1):
                                x_den_tmp_i1 = tf.layers.dense(inputs=x_d_i1_flat, units=128, name='seg_pred',
                                                               activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                                x_den_tmp_i1 = x_d_i1_flat

                            # negative local regions in feature map (f_a2_i) from image (x_a2_i)
                            x_d_tmp_i2 = tf.gather(x_num_i2, [neg_samples_index_list[local_neg_index, 0],
                                                              neg_samples_index_list[local_neg_index, 0] + 1,
                                                              neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_d_tmp_i2 = tf.gather(x_d_tmp_i2, [neg_samples_index_list[local_neg_index, 1],
                                                                neg_samples_index_list[local_neg_index, 1] + 1,
                                                                neg_samples_index_list[local_neg_index, 1] + 2], axis=2)
                            x_d_i2_flat = tf.layers.flatten(inputs=x_d_tmp_i2)
                            if (wgt_en == 1):
                                x_den_tmp_i2 = tf.layers.dense(inputs=x_d_i2_flat, units=128, name='seg_pred',
                                                               activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                                x_den_tmp_i2 = x_d_i2_flat

                            # negative local regions in feature map (f_a1_j) from image (x_a1_j)
                            x_d_tmp_i3 = tf.gather(x_num_i3, [neg_samples_index_list[local_neg_index, 0],
                                                              neg_samples_index_list[local_neg_index, 0] + 1,
                                                              neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_d_tmp_i3 = tf.gather(x_d_tmp_i3, [neg_samples_index_list[local_neg_index, 1],
                                                                neg_samples_index_list[local_neg_index, 1] + 1,
                                                                neg_samples_index_list[local_neg_index, 1] + 2], axis=2)
                            x_d_i3_flat = tf.layers.flatten(inputs=x_d_tmp_i3)
                            if (wgt_en == 1):
                                x_den_tmp_i3 = tf.layers.dense(inputs=x_d_i3_flat, units=128, name='seg_pred',
                                                               activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                                x_den_tmp_i3 = x_d_i3_flat

                            # negative local regions in feature map (f_a1_k) from image (x_a1_k)
                            x_d_tmp_i4 = tf.gather(x_num_i4, [neg_samples_index_list[local_neg_index, 0],
                                                              neg_samples_index_list[local_neg_index, 0] + 1,
                                                              neg_samples_index_list[local_neg_index, 0] + 2], axis=1)
                            x_d_tmp_i4 = tf.gather(x_d_tmp_i4, [neg_samples_index_list[local_neg_index, 1],
                                                                neg_samples_index_list[local_neg_index, 1] + 1,
                                                                neg_samples_index_list[local_neg_index, 1] + 2], axis=2)
                            x_d_i4_flat = tf.layers.flatten(inputs=x_d_tmp_i4)
                            if (wgt_en == 1):
                                x_den_tmp_i4 = tf.layers.dense(inputs=x_d_i4_flat, units=128, name='seg_pred',
                                                               activation=None, use_bias=False, reuse=tf.AUTO_REUSE)
                            else:
                    ...
                    local_loss = local_loss - tf.log(tf.math.reduce_sum(tf.exp(num_i1_i2_ss)) / (
                                tf.math.reduce_sum(tf.exp(num_i1_i2_ss)) + tf.math.reduce_sum(den_i1_ss)))
                    # local loss from feature map f_a1_i and f_a1_j
                    # loss from img-i vs i+4 (1a vs 2a)
                    local_loss = local_loss - tf.log(tf.math.reduce_sum(tf.exp(num_i1_i3_ss)) / (
                                tf.math.reduce_sum(tf.exp(num_i1_i3_ss)) + tf.math.reduce_sum(den_i2_ss)))
                    # local loss from feature map f_a1_i and f_a1_k
                    # loss from img-i vs i+8 (1a vs 3a)
                    local_loss = local_loss - tf.log(tf.math.reduce_sum(tf.exp(num_i1_i4_ss)) / (
                                tf.math.reduce_sum(tf.exp(num_i1_i4_ss)) + tf.math.reduce_sum(den_i3_ss)))
        ...
        if (inf == 0):
            ...
            with tf.control_dependencies(update_ops):
                ...
                optimizer_unet_dec = tf.train.AdamOptimizer(learning_rate=learn_rate_seg).minimize(cost_net,
                                                                                                   var_list=dec_net_vars)
        ...
        if (inf == 1):
            ...
            return {'x': x, 'train_phase': train_phase, 'reg_cost': cost_net, 'optimizer_unet_dec': optimizer_unet_dec, \
                    'train_summary': train_summary, 'y_pred': y_fin, 'val_totalc': val_totalc,
                    'val_summary': val_summary}
...
#         # 2x 3x3 conv and 1 maxpool
#         # Level 1
#         enc_c1_a = tf.nn.conv2d(ip_layer=x, name='enc_c1_a', num_filters=no_filters[1], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c1_b = tf.nn.conv2d(ip_layer=enc_c1_a, name='enc_c1_b', num_filters=no_filters[1], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c1_pool = layers.max_pool_layer2d(enc_c1_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",name='enc_c1_pool')
#         enc_layers_list.append(enc_c1_b)
#
#         # Level 2
#         enc_c2_a = tf.nn.conv2d(ip_layer=enc_c1_pool, name='enc_c2_a', num_filters=no_filters[2], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c2_b = tf.nn.conv2d(ip_layer=enc_c2_a, name='enc_c2_b', num_filters=no_filters[2], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c2_pool = layers.max_pool_layer2d(enc_c2_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",name='enc_c2_pool')
#         enc_layers_list.append(enc_c2_b)
#
#         # Level 3
#         enc_c3_a = tf.nn.conv2d(ip_layer=enc_c2_pool, name='enc_c3_a', num_filters=no_filters[3], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c3_b = tf.nn.conv2d(ip_layer=enc_c3_a, name='enc_c3_b', num_filters=no_filters[3], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c3_pool = layers.max_pool_layer2d(enc_c3_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",name='enc_c3_pool')
#         enc_layers_list.append(enc_c3_b)
#
#         # Level 4
#         enc_c4_a = tf.nn.conv2d(ip_layer=enc_c3_pool, name='enc_c4_a', num_filters=no_filters[4], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c4_b = tf.nn.conv2d(ip_layer=enc_c4_a, name='enc_c4_b', num_filters=no_filters[4], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c4_pool = layers.max_pool_layer2d(enc_c4_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",name='enc_c4_pool')
#         enc_layers_list.append(enc_c4_b)
#
#         # Level 5 - 2x Conv
#         enc_c5_a = tf.nn.conv2d(ip_layer=enc_c4_pool, name='enc_c5_a', num_filters=no_filters[5], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c5_b = tf.nn.conv2d(ip_layer=enc_c5_a, name='enc_c5_b', num_filters=no_filters[5], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c5_pool = layers.max_pool_layer2d(enc_c5_b, kernel_size=(2, 2), strides=(2, 2), padding="SAME",name='enc_c5_pool')
#         enc_layers_list.append(enc_c5_b)
#
#         # Level 6 - 2x Conv
#         enc_c6_a = tf.nn.conv2d(ip_layer=enc_c5_pool, name='enc_c6_a', num_filters=no_filters[5], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#         enc_c6_b = tf.nn.conv2d(ip_layer=enc_c6_a, name='enc_c6_b', num_filters=no_filters[5], use_relu=True,use_batch_norm=True, training_phase=train_phase)
#
#         if(encoder_list_return==1):
#             return enc_c6_b,enc_layers_list
#         else:
...
#         dec_c6_up = layers.upsample_layer(ip_layer=enc_c6_b, method=self.interp_val, scale_factor=int(scale_fac))
#         #print('dec 2 large up',dec_c6_up)
#         dec_dc6 = tf.nn.conv2d(ip_layer=dec_c6_up,name='dec_dc6', kernel_size=(fs_de,fs_de),num_filters=no_filters[5], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_cat_c6 = tf.concat((dec_dc6,enc_c5_b),axis=3,name='dec_cat_c6')
#         dec_c5_a = tf.nn.conv2d(ip_layer=dec_cat_c6,name='dec_c5_a', num_filters=no_filters[5], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_c5_b = tf.nn.conv2d(ip_layer=dec_c5_a,name='dec_c5_b', num_filters=no_filters[5], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         dec_c5_up = layers.upsample_layer(ip_layer=dec_c5_b, method=self.interp_val, scale_factor=int(scale_fac))
#         #print('dec large up',dec_c6_up,dec_c5_up)
#         dec_dc5 = tf.nn.conv2d(ip_layer=dec_c5_up,name='dec_dc5', kernel_size=(fs_de,fs_de),num_filters=no_filters[4], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_cat_c5 = tf.concat((dec_dc5,enc_c4_b),axis=3,name='dec_cat_c5')
#         dec_c4_a = tf.nn.conv2d(ip_layer=dec_cat_c5,name='dec_c4_a', num_filters=no_filters[4], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_c4_b = tf.nn.conv2d(ip_layer=dec_c4_a,name='dec_c4_b', num_filters=no_filters[4], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         # Level 4
#         dec_up4 = layers.upsample_layer(ip_layer=dec_c4_b, method=self.interp_val, scale_factor=scale_fac)
#         dec_dc4 = tf.nn.conv2d(ip_layer=dec_up4,name='dec_dc4', kernel_size=(fs_de,fs_de),num_filters=no_filters[3], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_cat_c4 = tf.concat((dec_dc4,enc_c3_b),axis=3,name='dec_cat_c4')
#         dec_c3_a = tf.nn.conv2d(ip_layer=dec_cat_c4,name='dec_c3_a', num_filters=no_filters[3], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_c3_b = tf.nn.conv2d(ip_layer=dec_c3_a,name='dec_c3_b', num_filters=no_filters[3], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         # Level 3
#         dec_up3 = layers.upsample_layer(ip_layer=dec_c3_b, method=self.interp_val, scale_factor=scale_fac)
#         dec_dc3 = tf.nn.conv2d(ip_layer=dec_up3,name='dec_dc3', kernel_size=(fs_de,fs_de),num_filters=no_filters[2],use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_cat_c3 = tf.concat((dec_dc3,enc_c2_b),axis=3,name='dec_cat_c3')
#         dec_c2_a = tf.nn.conv2d(ip_layer=dec_cat_c3,name='dec_c2_a', num_filters=no_filters[2], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_c2_b = tf.nn.conv2d(ip_layer=dec_c2_a,name='dec_c2_b', num_filters=no_filters[2], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         # Level 2
#         dec_up2 = layers.upsample_layer(ip_layer=dec_c2_b, method=self.interp_val, scale_factor=scale_fac)
#         dec_dc2 = tf.nn.conv2d(ip_layer=dec_up2,name='dec_dc2', kernel_size=(fs_de,fs_de),num_filters=no_filters[1], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         dec_cat_c2 = tf.concat((dec_dc2,enc_c1_b),axis=3,name='dec_cat_c2')
#         dec_c1_a = tf.nn.conv2d(ip_layer=dec_cat_c2,name='dec_c1_a', num_filters=no_filters[1], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         # Level 1
#         seg_c1_a = tf.nn.conv2d(ip_layer=dec_c1_a,name='seg_c1_a',num_filters=no_filters[1], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         seg_c1_b = tf.nn.conv2d(ip_layer=seg_c1_a,name='seg_c1_b', num_filters=no_filters[1], use_relu=True, use_batch_norm=True, training_phase=train_phase)
#
#         #Final output layer - Logits before softmax
#         seg_fin_layer = tf.nn.conv2d(ip_layer=seg_c1_b,name='seg_fin_layer', num_filters=self.num_classes,use_bias=False, use_relu=False, use_batch_norm=False, training_phase=train_phase)
#         actual_cost = loss.dice_loss_with_backgrnd(logits=seg_fin_layer, labels=y_l_onehot)
#
#         # Predict Class
#         y_pred = tf.nn.softmax(seg_fin_layer)
...
#
#         #g_2 small network with two 1x1 convolutions
#         seg_c1_a = tf.nn.conv2d(ip_layer=tmp_dec_layer,name='seg_c1_a', kernel_size=(1,1), num_filters=tmp_no_filters,use_bias=False, use_relu=True, use_batch_norm=True, training_phase=train_phase)
#         seg_c1_b = tf.nn.conv2d(ip_layer=seg_c1_a, name='seg_c1_b', kernel_size=(1, 1),num_filters=tmp_no_filters, use_bias=False, use_relu=False,use_batch_norm=False, training_phase=train_phase)
#
#         y_fin_tmp=seg_c1_b
#         #print('y_fin',y_fin_tmp)
#

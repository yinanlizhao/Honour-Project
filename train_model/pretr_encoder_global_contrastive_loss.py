...
save_dir = str(cfg.srt_dir) + '/models/' + str(
    parse_config.dataset) + '/trained_models/pretrain_encoder_with_global_contrastive_loss/'
...
#     save_dir = str(save_dir) + 'n_a/'
# else:
#     save_dir = str(save_dir) + 'w_a/'
#
# save_dir = str(save_dir) + 'g_' + str(parse_config.global_loss_exp_no) + '_n_' + str(
#     parse_config.n_parts) + '/'
#
# save_dir = str(save_dir) + 't_' + str(parse_config.temp_fac) + '/'
#
# save_dir = str(save_dir) + str(parse_config.no_of_tr_imgs) + '/' + str(parse_config.comb_tr_imgs) + '_v' + str(
#     parse_config.ver) + '/e_' + str(parse_config.bbox_dim) + '_n_' + str(
#     parse_config.n_iter) + '_l_' + str(parse_config.lr_reg) + '/'
...
#
# save_dir = str(save_dir) + 'global_loss_exp_no_' + str(parse_config.global_loss_exp_no) + '_n_parts_' + str(
#     parse_config.n_parts) + '/'
#
# save_dir = str(save_dir) + 'temp_fac_' + str(parse_config.temp_fac) + '/'
#
# save_dir = str(save_dir) + str(parse_config.no_of_tr_imgs) + '/' + str(parse_config.comb_tr_imgs) + '_v' + str(
#     parse_config.ver) + '/enc_bbox_dim_' + str(parse_config.bbox_dim) + '_n_iter_' + str(
#     parse_config.n_iter) + '_lr_reg_' + str(parse_config.lr_reg) + '/'
...
checkpoint_filename = 't_' + str(parse_config.dataset)
...
for epoch_i in range(start_epoch, n_epochs):
    ...
    if (parse_config.global_loss_exp_no == 1):
        ...
        color_batch1_v2 = sess.run(ae_rc['rd_fin'],
                                   feed_dict={ae_rc['x_tmp']: crop_batch1[cfg.batch_size_ft:2 * cfg.batch_size_ft]})
        ...
        color_batch2_v2 = sess.run(ae_rc['rd_fin'],
                                   feed_dict={ae_rc['x_tmp']: crop_batch2[cfg.batch_size_ft:2 * cfg.batch_size_ft]})
        ...
        # (c) Set 2 of original images batch (different to set 1)
        # (d) Set 2's 2 different augmented versions concatenated (aug_batch2)
        cat_batch = stitch_batch_global_loss_gdnew(cfg, img_batch, color_batch1, color_batch2, img_batch_t2,
                                                   color_batch3, color_batch4, n_parts)
    ...
    if ((epoch_i == n_epochs - 1)):
        ...
        mp = str(save_dir) + str(checkpoint_filename) + '_' + str(epoch_i) + ".ckpt"
        saver.save(sess, mp)
        try:
            mp_best
        except NameError:
